    ...
    try {
        ...
        if ($action === 'add_category' && !empty($_POST['category_name'])) {
            ...
            if ($category->save()) {
                ...
                // Обработка загрузки изображения для категории
        ...
        if ($action === 'delete_category' && !empty($_POST['category_id'])) {
            ...
            if ($category) {
                if (!$category->hasSubcategories()) {
                    // Удаляем изображение категории, если есть
                    if ($category->hasImage()) {
                        $oldFile = $_SERVER['DOCUMENT_ROOT'] . $category->image_url;
                        if (file_exists($oldFile)) {
                            unlink($oldFile);
    ...
    <div class="category">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <div>
                ...
                <!-- Форма для обновления изображения (скрыта по умолчанию) -->
...
<script>
    // Функция для показа/скрытия формы загрузки изображения
